...
func backgroundWorker(rdb *redis.Client, db *gorm.DB) {
	...
	var WAIT_INTERVAL = (33 * time.Second)
	var cursor uint64 = 0
	var matchPattern = "dtac-transaction-callback-api:*" // Pattern to match keys
	var count = int64(100)                               // Limit to 100 keys per scan

	fmt.Println("##### DTAC TRANSACTION WORKER RUNNING DON'T CLOSE TERMINAL #####")
...
func threadWorker(id int, wg *sync.WaitGroup, jsonString string, rdb *redis.Client, ctx context.Context, db *gorm.DB) error {
	...
	type dtac_transaction_logs struct {
	...
	insertId := uuid.New()
	logEntry := dtac_transaction_logs{
		ID:            insertId.String(),
		Code:          transactionData.Code,
	...
	redis_set_key := "dtac-transaction-log-worker:" + transactionData.TranRef
	...
	redis_del_key := "dtac-transaction-callback-api:" + transactionData.TranRef
	rdb.Del(ctx, redis_del_key).Result()
